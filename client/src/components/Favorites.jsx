...
import { Trash2, BookmarkPlus, Eye, CircleCheckBig } from "lucide-react";
...
const Favorites = () => {
  ...
  const handleWatchlist = async (movieId, movieTitle) => {
    try {
      const res = await axios.post("http://localhost:5000/movie/watchlist", { movieId, userId, movieTitle });
  ...
  const handleWatched = async (movieId, movieTitle) => {
    try {
      const res = await axios.post("http://localhost:5000/movie/watched", { movieId, userId, movieTitle });
  ...
  const removeFavorite = async (movieId, movieTitle) => {
    try {
      const res = await axios.delete("http://localhost:5000/movie/removefavorite", {
        params: { userId, movieId, movieTitle },
      });
      if (res.status === 200) {
        toast.success("Removed from Favorites ");
        setFavoriteMovieId((prev) => prev.filter((fav) => fav.movie_id !== movieId));
  ...
  return (
    <div className="relative h-full flex flex-col p-1 sm:p-4">
      <div className="sticky top-0  pb-1">
        <h2 className="text-3xl font-semibold text-blue-400 mb-2"> Your Favorites </h2>
      </div>
